...
namespace FAL.Services
{
    public class S3Service : IS3Service
    {
        ...
        public async Task<bool> AddFileToS3Async(IFormFile file, string fileName, string bucketName, TypeOfRequest type,string mediaId,string systemId, string userId = null)
        {
            try
            {
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    if (file.Length < _divideSize)
                    {
                        ...
                        uploadRequest = GetMetaData(uploadRequest, type, userId, file, width, height,mediaId,systemId);
        ...
        private TransferUtilityUploadRequest GetMetaData(TransferUtilityUploadRequest request, TypeOfRequest type, string userId, IFormFile file, int width, int height,string mediaId,string systemId)
        {
            try
            {
                request.Metadata.Add("SystemId", systemId);
